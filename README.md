...
├── public/
│   ├── articles/                   # 文章目录（自动扫描）
│   │   ├── categories.json         # 分类配置（id、name、icon、image、tags）
│   │   ├── frontend/               # 分类目录，内放 {id}_{title}.html 文章
│   │   └── backend/
│   ├── icons/                      # 分类图标（categories.json 的 image 字段引用）
│   └── favicon.svg                 # Logo
├── src/
...
将文章文件放入对应分类目录，命名格式为 `{id}_{title}.html` 或 `{id}_{title}.md`：
...
2. 在 `public/articles/categories.json` 添加分类配置：
   ...
   {
     "categories": [
       {"id": "frontend", "name": "前端开发", "icon": "💻", "image": "frontend.svg", "tags": ["前端"]}
     ]
   }
   ```
3. 将分类图标放入 `public/icons/` 目录，`image` 字段填写文件名

`image` 字段支持三种写法：文件名（`frontend.svg` → 解析为 `/icons/frontend.svg`）、`/` 开头的 public 路径、完整 URL。

### 3. 覆盖文章元数据（可选）

如需为个别文章指定不同的 `date`、`tags` 等，在分类目录下创建 `meta.json`：
